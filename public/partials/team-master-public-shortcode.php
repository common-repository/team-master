...
$background = !empty($atts['background']) ? $atts['background'] : get_option('tm_default_background_color');
...
if ($template == 1) {
    if ($tmposts) {
        ...
        foreach ($tmposts as $key => $tmpost) {
            ...
            require plugin_dir_path(dirname(__FILE__)) . 'partials/templates/tm_template_1.php';
        }
        echo '</div>';
...
if ($template == 2) {
    if ($tmposts) {
        ...
        foreach ($tmposts as $tmpost) {
            ...
            require plugin_dir_path(dirname(__FILE__)) . 'partials/templates/tm_template_2.php';
        }
        echo '</div>';
        if ($num_of_pages > 1 && $loadMore == 'yes' && $carousel == 'no') {
            ?>
...
if ($template == 3) {
    if ($tmposts) {
        ...
        foreach ($tmposts as $tmpost) {
            ...
            require plugin_dir_path(dirname(__FILE__)) . 'partials/templates/tm_template_3.php';
        }
        echo '</div>';
        if ($num_of_pages > 1 && $loadMore == 'yes' && $carousel == 'no') {
            ?>
...
if ($template == 4) {
    if ($tmposts) {
        ...
        foreach ($tmposts as $key => $tmpost) {
            ...
            $url = get_the_permalink($tmpost->ID);
            $member_social_media = maybe_unserialize(get_post_meta($tmpost->ID, 'member_social_media', true));
            require plugin_dir_path(dirname(__FILE__)) . 'partials/templates/tm_template_4.php';
        }
        echo '</div>';
        if ($num_of_pages > 1 && $loadMore == 'yes' && $carousel == 'no') {
            ?>
...
if ($template == 'custom') {
    if ($tmposts) {
        ...
        echo '<div id="tm_template_' . $template . '" class="row loadmoremembersrow ' . $carouselClass . '" ' . $sliderData . '>';
        foreach ($tmposts as $key => $tmpost) {
            require plugin_dir_path(dirname(__FILE__)) . 'partials/templates/tm_template_custom.php';
        }
        echo '</div>';
        if ($num_of_pages > 1 && $loadMore == 'yes' && $carousel == 'no') {
            ?>
